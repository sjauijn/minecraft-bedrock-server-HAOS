...
SESSION_COOKIE_NAME="mcserver_ha_session"
...
@app.route("/api/op_by_name", methods=["POST"])
def api_op_by_name():
    payload = request.get_json(silent=True) or request.form
    name = (payload.get("name") or "").strip()
    action = (payload.get("action") or "op").strip().lower()

    if not name:
        return jsonify({"ok": False, "message": "Player name is required"}), 400

    if action not in ("op", "deop"):
        return jsonify({"ok": False, "message": "action must be 'op' or 'deop'"}), 400

    if get_server_status() != "running":
        return (
            jsonify({"ok": False, "message": "Server must be running to change permissions"}),
            409,
        )

    try:
        result = subprocess.run(
            [SEND_COMMAND_BIN, action, name],
            check=False,
            capture_output=True,
            text=True,
            timeout=10,
        ...
        return jsonify({"ok": False, "message": f"Failed to send command: {e}"}), 500

    if result.returncode != 0:
        detail = (result.stderr or result.stdout or "").strip()
        return (
            jsonify({"ok": False, "message": f"send-command failed: {detail}"}),
            500,
        )

    verb = "opped" if action == "op" else "de-opped"
    return jsonify({"ok": True, "message": f"Sent '{action} {name}' — player will be {verb} on next check."})
      ...
      <div class="card bg-dark border-secondary">
        ...
        <div class="card-body">
          ...
          <div class="mb-3 p-2 border border-secondary rounded">
            <label class="form-label mb-1">Op by player name (offline mode)</label>
            <div class="form-text text-muted mb-2">
              When "Online mode (Xbox Live authentication)" is off, players connect without a XUID,
              so they can't be added via the XUID field above. This sends the server's built-in
              <code>op &lt;name&gt;</code> / <code>deop &lt;name&gt;</code> console commands instead,
              which match by player name. The server must be running, and the player name must match
              exactly (case-sensitive) what appears in-game / in the logs.
            </div>
            <div class="input-group input-group-sm">
              <input type="text" class="form-control bg-black text-light" id="op_by_name_input" placeholder="e.g. Алекс">
              <button class="btn btn-success" type="button" onclick="opByName('op')">Op</button>
              <button class="btn btn-outline-danger" type="button" onclick="opByName('deop')">De-op</button>
            </div>
            <div class="form-text" id="op_by_name_status"></div>
          </div>
...
<script>
  ...
  function opByName(action) {
    const input = document.getElementById('op_by_name_input');
    const statusEl = document.getElementById('op_by_name_status');
    ...
    fetch('api/op_by_name', {
      ...
      .then(({ ok, data }) => {
        statusEl.textContent = data.message || (ok ? 'Done.' : 'Failed.');
        statusEl.className = ok ? 'form-text text-success' : 'form-text text-danger';
        if (ok) {
          input.value = '';
